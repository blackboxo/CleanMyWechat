...
本次版本主要围绕“更简单、更安全、更容易确认”做了更新：

1. 重新整理首页和清理设置界面，保留常用选项，减少用户理解成本。
2. 清理前增加确认弹窗，以表格方式展示类型、大小和完整路径。
3. 支持新版微信目录，包括 `xwechat_files`、`MsgAttach`、小程序缓存、日志和系统缓存目录。
4. 视频目录支持按月份快速处理；视频目录下的缩略图和附属文件会跟随视频一起清理。
5. 设置中新增“直接删除，不移动到回收站”、开机自启动和定期自动清理。
6. 自动清理只在开机启动场景触发，手动打开 App 不会自动开始清理。
7. 扫描和清理会跳过程序组件、数据库文件和白名单内容，降低误删风险。
